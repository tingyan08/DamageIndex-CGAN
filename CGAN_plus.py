...
class CGAN_plus(object):
    def __init__(self, args):
        ...
        self.D = discriminator(input_dim=data[0].shape[1], output_dim=self.class_num + 1, input_size=self.input_size, class_num=self.class_num)
        ...
        if self.gpu_mode:
            ...
            self.MCE_Loss = MCE_Loss().cuda()
            self.MSE_Loss = nn.MSELoss().cuda()
        else:
            self.BCE_loss = nn.BCELoss()
            self.MCE_Loss = MCE_Loss()
    ...
    def seperate(self, info):
        cont_d = sum(self.data_loader.dataset.n_each_task)
        cont = info[:, cont_d:]
        disc = info[:, :cont_d]
        return disc, cont
    ...
    def train(self):
        ...
        for epoch in range(self.epoch):
            ...
            for iter, data in enumerate(self.data_loader):
                ...
                y_ = torch.concat([y_disc_, y_cont_], axis=1)
                ...
                y_fill_ = y_vec_.unsqueeze(2).unsqueeze(3).expand(self.batch_size, self.class_num, self.input_size, self.input_size)
                if self.gpu_mode:
                    x_, z_, y_vec_, y_fill_, y_disc_, y_cont_ = x_.cuda(), z_.cuda(), y_vec_.cuda(), y_fill_.cuda(), y_disc_.cuda(), y_cont_.cuda()

                # update D network
                self.D_optimizer.zero_grad()

                D_real, info_real = self.D(x_, y_fill_)
                D_real_loss = self.BCE_loss(D_real, self.y_real_)
                D_disc, D_cont = self.seperate(info_real)
                D_cont_loss = self.MSE_Loss(D_cont, y_cont_)
                D_disc_loss = self.MCE_Loss(self.data_loader.dataset.n_each_task, D_disc, y_disc_)

                G_ = self.G(z_, y_vec_)
                D_fake, _= self.D(G_, y_fill_)
                D_fake_loss = self.BCE_loss(D_fake, self.y_fake_)

                if torch.isnan(D_cont_loss): 
                    D_loss = D_real_loss + D_fake_loss + D_disc_loss
                elif D_disc_loss == 0: 
                    D_loss = D_real_loss + D_fake_loss + D_cont_loss
                else:
                    D_loss = D_real_loss + D_fake_loss + D_cont_loss + D_disc_loss
                ...
                G_ = self.G(z_, y_vec_)
                D_fake, info_fake = self.D(G_, y_fill_)
                G_disc, G_cont = self.seperate(info_fake)
                G_cont_loss = self.MSE_Loss(G_cont, y_cont_)
                G_disc_loss = self.MCE_Loss(self.data_loader.dataset.n_each_task, G_disc, y_disc_)

                if torch.isnan(G_cont_loss): 
                    G_loss = self.BCE_loss(D_fake, self.y_real_) + G_disc_loss
                elif G_disc_loss == 0: 
                    G_loss = self.BCE_loss(D_fake, self.y_real_) + G_cont_loss 
                else: 
                    G_loss = self.BCE_loss(D_fake, self.y_real_) + G_cont_loss + G_disc_loss
